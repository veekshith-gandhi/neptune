import { SubmitBasicHotelInfoDTO, SubmitHotelRoomInfoDTO } from "../@types/dto/hotel-form-dto";
import { HotelEndPoint, HotelImageEndPoint, HotelRoomsEndPoint } from "../constants/api-endpoints";
...
class HotelFormApiService extends BaseApiService {
	...
	submitHotelRoomInformation = <T = any>(payload: SubmitHotelRoomInfoDTO, id: string) => {
		if (!id) {
	...
	submitHotelRoomPriceInformation = <T = any>(payload: SubmitHotelRoomInfoDTO, id: string) => {
		if (!id) {
			return this.axiosInstance.post<T>(HotelRoomsEndPoint, payload);
		}
		return this.axiosInstance.patch<T>(HotelRoomsEndPoint + `${id}/`, payload);
