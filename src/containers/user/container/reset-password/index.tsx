...
import { clearResetPassword, clearValidateToken, resetPassword, validateToken } from "../../redux/action";
...
export const ResetPassword: FunctionComponent = () => {
	...
	const [invalid, setInvalid] = useState({ newPassword: false, confirmPassword: false, confirmPasswordNotMatch: false, passwordNotStrong: false });
	...
	const validate = (): boolean => {
		...
		if (newPasswordRef.current?.value && (newPasswordRef.current?.value.length < 6)) {
			...
			<div className="login_box">
				...
				<div className="login_input_box mb-3">
					...
					<div className="login_input_container position-relative show_password_input_container">
						<input onChange={() => setInvalid({ ...invalid, newPassword: false, passwordNotStrong: false })} ref={newPasswordRef} type={showNewPassword ? "text" : "password"}/>
						{invalid.newPassword ? <span className="invalid"><I18 tkey="Please enter password" /></span> : ""}
						{!invalid.confirmPassword && invalid.passwordNotStrong ? <span className="invalid"><I18 tkey="Password must be higher than 6" /></span> : ""}
						{showNewPassword ? 
							<span onClick={() => setShowNewPassword(!showNewPassword)} className="show_password"><EyeInvisibleOutlined /></span> 
							: 
							<span onClick={() => setShowNewPassword(!showNewPassword)} className="show_password"><EyeOutlined /></span>
						}
					</div>
				</div>
				<div className="login_input_box mb-3">
					...
					<div className="login_input_container position-relative show_password_input_container">
						<input onChange={() => setInvalid({ ...invalid, confirmPassword: false, confirmPasswordNotMatch: false })} ref={confirmPasswordRef} type={showPassword ? "text" : "password"}/>
