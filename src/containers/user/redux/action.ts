...
export const verify = (token: string) => ({
  type: Events.VERIFY,
  payload: axios.post(`${config.API_URL}/account/email-verify`, {
    token: token,
  }),
});
...
export const forgotPassword = (forgotDetails: ForgotPasswordData) => ({
  ...
  payload: axios.post(
    `${config.API_URL}/account/request-reset-email`,
    forgotDetails
  ),
});
...
export const validateToken = (validateTokenDetails: ValidateTokenDetails) => ({
  ...
  payload: axios.get(
    `${config.API_URL}/account/password-reset/${validateTokenDetails.uidb64}/${validateTokenDetails.token}/`
  ),
});
...
export const resetPassword = (resetDetails: ResetPasswordDetails) => ({
  ...
  payload: axios.post(
    `${config.API_URL}/account/password-reset-complete`,
    resetDetails
  ),
});
