...
import { Button, Card, Col, Collapse, DatePicker, Form, Input, InputNumber, notification, Progress, Row, Select, Space, Typography } from "antd";
import moment from "moment";
import { FunctionComponent, useState } from "react";
import { useDispatch } from "react-redux";
import { submitHotelRoomInformation, submitHotelRoomPriceInformation } from "../../../../../services/hotel-form-api-service";
import { useAppSelector } from "../../../../../store";
import { addHotelRoomsCreation, addHotelRoomsPriceCreation, addSubmitedIdToRoom } from "../redux/action";
...
export const RoomsAndSpacesDetails : FunctionComponent = () => {
	...
	return (
		<Card id="rooms-spaces-ref" style={{ width: "140vh", margin: "auto" }}>
			...
			<Form style={{ padding: "25px 25px 0 25px" }} {...layout} layout="vertical" onFinish={onFinishRooms}>
				<Collapse
				...
				>
					<Panel header="Room Details" key="1" style={customPanelStyle}>
						<div style={{ padding: 24, background: "aliceblue" }}>
							...
							<Row gutter={[8, 24]}>
								<Col span={12}>
									<Form.Item label="Rooms Available" name={["availablerooms"]} rules={[{ required: true }]}>
							...
							<Row gutter={[2, 2]}>
								<Col span={12}>
									<Form.Item label="Smoking allowed" name={["smokingallowed"]} rules={[{ required: true }]}>
										<Select placeholder="select" allowClear>
							...
							<Row style={{ width: 620 }}>
								<Col span={6}>
									<Form.Item name={["length"]} rules={[{ required: true }]}>
										<InputNumber placeholder="Length" name="length" controls={false} />
									</Form.Item>
								</Col>
								<Col span={6}>
									<Form.Item name={["breadth"]} rules={[{ required: true }]}>
										<InputNumber placeholder="Breadth" name="breadth" controls={false} />
									</Form.Item>
								</Col>
								<Col span={6}>
							...
							<Row>
								<Col span={12}>
									<Form.Item name={["extrabed"]} label="Extra Bed" rules={[{ required: true }]}>
			...
			<Form style={{ padding: "0 25px 0 25px" }} {...layout} layout="vertical" onFinish={onFinishBaseRooms}>
				<Collapse
					expandIconPosition="right"
					expandIcon={({ isActive }) => <CaretRightOutlined rotate={isActive ? -270 : -180} />}
					bordered={false}
					style={{ background: "#FFFFFF" }}
				>
					<Panel header="Base Room Price and Availablity" key="2" style={customPanelStyle}>
						<div style={{ padding: 24, background: "aliceblue" }}>
							<Row gutter={[8, 24]}>
								<Col span={12}>
							...
							<Row gutter={[8, 24]}>
								...
								<Col span={12}>
									<Form.Item label="Max no of guest allowed" name={["noguestallowed"]} rules={[{ required: true }]}>
							...
							<Row gutter={[2, 2]}>
								<Col span={12}>
									<Form.Item
										label={<p style={{ fontSize: 12 }}>Price for child (0-5yrs)</p>}
										name={["pricerange1"]}
										rules={[{ required: true }]}
									>
								...
								<Col span={12}>
									<Form.Item
										label={<p style={{ fontSize: 11 }}>Price for child (6-12yrs)</p>}
										name={["pricerange2"]}
										rules={[{ required: true }]}
									>
							...
							<Row>
								<Col span={18}>
									<Form.Item
										...
										label={
											<p style={{ fontSize: 10 }}>
                                                Select date range for which your property is available for booking for guest
											</p>
										}
									>
										<RangePicker
											onChange={(values: any) => {
												const val1 = values ? moment(values[0])?.format("DD-MM-YYYY") : "";
												const val2 = values ? moment(values[1])?.format("DD-MM-YYYY") : "";
												setDateFrom(val1);
												setDateTo(val2);
											}}
										/>
