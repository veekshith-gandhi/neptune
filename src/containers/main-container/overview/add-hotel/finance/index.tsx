...
export const FinanceDetails : FunctionComponent = () => {
	...
	return (
		<Card style={{ width: "140vh", margin: "auto" }}>
			<div id="finance-legal-ref" style={{ display: "flex", justifyContent: "space-between" }}>
				<div>
			...
			<Form style={{ padding: 24, minHeight: 360 }} {...layout} layout="vertical" onFinish={onFinish}>
				...
				>
					<Panel header="Bank Details" key="1" style={customPanelStyle}>
						<div style={{ padding: 24, background: "aliceblue" }}>
							<Form.Item
								...
								name={["acountnumber"]}
								rules={[{ required: true, pattern: new RegExp(/^[0-9]+$/), message: "Please input your account number!" }]}
							>
								<Input maxLength={12} minLength={12} name="acountnumber" required type="tel" />
							</Form.Item>
							<Space>
								<Form.Item
									label="IFSC Code"
									name={["state"]}
									rules={[
										{ required: true, pattern: new RegExp(/^[A-Za-z]{4}\d{7}$/), message: "Please input your account number!" }
									]}
								>
									<Input allowClear maxLength={11} required />
								</Form.Item>
								<Form.Item label="Bank Name" style={{ width: "300px" }} hasFeedback name="starRating">
					...
					<Panel header="Pan Details" key="2" style={customPanelStyle}>
						<div style={{ padding: 24, background: "aliceblue" }}>
							...
							<Form.Item label="DOB" name={["date"]} rules={[{ required: true }]}>
				...
				<Form.Item name={["checked"]} rules={[{ required: true }]} valuePropName="checked">
					<Checkbox onChange={checkBoxChecking} name="checked" value={"checked"}>
